import socket
import sys
import time
import math as m
# Comprobamos numero de argumentos
n_arg = len(sys.argv)
if(n_arg!=4):
    sys.exit('Numero de argumentos erroneo')

# Creamos el socket TCP
sockfd = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

# IP del servidor y puerto
host = str(sys.argv[2])
ip = socket.gethostbyname(host)
puerto = int(sys.argv[3])
server_address = (ip, puerto)

# Numero de peticiones , auxiliar para bucle,numero de secuencia
num_pet = int(sys.argv[1])
num_pet2 = num_pet
nseq = 1
ttl = 44
times= 100
minimo = 100
maximo = 0
avg = 0
mdev = 0
#Realizamos el connect
sockfd.connect(server_address)
print("PING {} ({}) {} bytes of data. ".format(host, ip, num_pet*64))

# Iteramos 
while num_pet2>0:
    linea = "PING"
    
    start = time.time() 
    sockfd.sendall(linea.encode('utf-8'))   # Realizamos un write
    sockfd.recv(64)  #Recibimos bloques de 64 bytes
    end = time.time()
    times = (end - start)*1000 #pasamos el tiempo a milisegundos
    if(minimo>times):
        minimo=times
    if(maximo<times):
        maximo=times
    avg = avg + times
    print("64 bytes from {} ({}): icmp_seq={} ttl={} time={}".format(host,ip,nseq,ttl,times))
    nseq = nseq + 1
    num_pet2 = num_pet2 - 1

mdev = m.sqrt(avg**2/num_pet - (avg/num_pet)**2)
print("-------------------------- {} ping statistics------------------------".format(host))
print("{} packets transmited, {} received, 0% packet loss, time {}ms".format(num_pet, num_pet,avg))
print("rtt min/avg/max/mdev = {}/{}/{}/{}".format(minimo,avg/num_pet,maximo,mdev))
sockfd.close()
